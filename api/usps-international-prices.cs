...
namespace UspsProcessor
{
    public class UspsInternationalPricesProcessor
    {
        ...
        #region GetInternationalPricesAsync
        public async Task<InternationalPriceLookupResponse> GetInternationalPricesAsync(InternationalPriceLookupRequest request)
        {
            InternationalPriceLookupResponse lookupResponse = new InternationalPriceLookupResponse
            {
                isSuccess = true,
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(token))
                {
                    ...
                    UspsGenericResponse genericResponse = await MakeRestRequestAsync("international-prices", payload);
                    if (genericResponse.isSuccess)
                    {
                        if (!string.IsNullOrWhiteSpace(genericResponse.message))
                        {
                            UspsInternationalPriceResponse priceResponse = JsonConvert.DeserializeObject<UspsInternationalPriceResponse>(genericResponse.message);
                            if (priceResponse != null && priceResponse.quotes != null && priceResponse.quotes.Count > 0)
                            {
                                foreach (UspsInternationalPriceQuote quote in priceResponse.quotes)
                                {
                                    lookupResponse.quotes.Add(new InternationalPriceQuote
                                    {
                            ...
                            else
                            {
                                ...
                                lookupResponse.errorDesc = "No international price quotes returned";
                            }
                        }
                        else
                        {
                            lookupResponse.isSuccess = false;
                            lookupResponse.errorDesc = "Blank response received from international prices call";
                        }
                    }
                    else
                    {
                        lookupResponse.isSuccess = false;
                        lookupResponse.errorDesc = !string.IsNullOrWhiteSpace(genericResponse.error) ? genericResponse.error : "Error processing international prices request";
                        _logger.Error("USPS International Prices Error = {0}", lookupResponse.errorDesc);
                    }
                }
                else
                {
        ...
        private async Task<UspsGenericResponse> MakeRestRequestAsync(string method, string payload)
        {
            ...
            try
            {
                ...
                else if (method == "international-prices")
                {
                    url = string.Format("{0}prices/v3/international", _url);
                    webRequest = (HttpWebRequest)WebRequest.Create(url);
                    string token = MemoryCache.Default["uspsToken"] as string ?? string.Empty;
    ...
    public sealed class InternationalPriceLookupRequest
    {
    ...
    public sealed class InternationalPriceLookupResponse
    {
        public bool isSuccess { get; set; }
        public string errorDesc { get; set; } = string.Empty;
        public List<InternationalPriceQuote> quotes { get; set; } = new List<InternationalPriceQuote>();
    }

    public sealed class InternationalPriceQuote
    {
    ...
    internal sealed class UspsInternationalPriceResponse
    {
        public List<UspsInternationalPriceQuote> quotes { get; set; } = new List<UspsInternationalPriceQuote>();
    }

    internal sealed class UspsInternationalPriceQuote
    {
        public string service { get; set; } = string.Empty;
        public decimal price { get; set; }
        public string currency { get; set; } = "USD";
